...
const apiKey = '575733e6a725ac45621a641d001d2063'
...
function updateDate() {
    if (currentDatetxt) {
        ...
        console.error("Element with class '.current-date-txt' not found.")
    }
}
...
async function updateForecastsInfo(city) {
    ...
    const todayDateStr = todayDate[0] 

    if (!forecastItemsContainer) {
        console.error("forecastItemsContainer element not found.")
        return
    }
    ...
    forecastsData.list.forEach(forecastWeather => {
        if (forecastWeather.dt_txt.includes(timeTaken) &&
            !forecastWeather.dt_txt.includes(todayDateStr)) {
